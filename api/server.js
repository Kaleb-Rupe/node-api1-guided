...
server.get(`/api/dogs:id`, async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: `Error fetching dogs ${req.params.id}: ${err.message}`,
    });
  }
});
...
server.put(`/api/dogs:id`, async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: `Error updating dog ${req.params.id}: ${err.message}`,
    });
  }
});
...
server.delete(`/api/dogs/:id`, async (req, res) => {
  try {
    ...
    if (!deletedDog) {
      ...
      res.status(200).json({
        message: `dog with id ${id} was successfully deleted`,
        data: deletedDog,
      });
    }
  } catch (err) {
    res.status(500).json({
      message: `Error deleting dog ${req.params.id}: ${err.message}`,
    });
  }
});
